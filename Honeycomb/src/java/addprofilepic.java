 ...
 * @author Reetam
 */
@WebServlet(urlPatterns = {"/addprofilepic"})
...
public class addprofilepic extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
            ...
            try{
                ...
                ps=con.prepareStatement("insert into Profileimage(username, profile_pic) values(?,?)");
                if(is!=null){
                    ps.setString(1, un);
                    ps.setBlob(2, is);
                    ps.executeUpdate();
                    response.sendRedirect("profile.jsp?msg=Profile_Picture_Updated");
                }
                else{
                    response.sendRedirect("profile.jsp?msg=Cannot_Update_Profile_Picture_Please_Try_Again");
                }
                ps.close();
                con.close();
            }
